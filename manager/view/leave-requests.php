        ...
        <table>
            ...
            <?php foreach ($query as $row): ?>
                <tr>
                    ...
                    <td>
                        <a href="<?php echo manager_url('approve_leave'. "?id=" . $row['id'] )?>"  class="btn">Onayla</a>
                        <a href="<?= manager_url('cancel_leave?id=' . $row['id']) ?>"  class="btn">İptal Et</a>
                    </td>
                </tr>
            <?php endforeach; ?>
            </tbody>
...
<?php if ($totalRecord > $pageLimit): ?>
    <div class="pagination">
        <ul>
            <?= $db->showPagination(manager_url(route(1) . '?' . $pageParam . '=[page]')) ?>
        </ul>
    </div>
<?php endif; ?>
